...
class tree(object):
    def __init__(self, labels, myDat):
        self.myTree = None
        self.labels = labels
        self.myDat = myDat
    ...
    # 划分数据集
    def splitDataSet(self, dataSet, axis, value):  # 待划分的数据集、划分数据集特征、需要返回的特征的值
    ...
    # Gain = H(D) - H(D|A)
    # Gain_ratio = Gain/IV
    # IV = -sum{prob*log2prob}
    def chooseBestFeatureToSplit(self, dataSet):
        numFeatures = len(dataSet[0]) - 1  # 计算特征属性的个数
        baseEntropy = self.calcShannonEnt(dataSet)  # 调用前面写好的calcShannonEnt函数，计算整个数据集的原始香农熵
        bestGain_ratio = 0.0  # 记录信息增益
        ...
        for i in range(numFeatures):  # 遍历每个属性
            ...
            for value in uniqueVals:  # 遍历该属性中的每个可能的属性值
                subDataSet = self.splitDataSet(dataSet, i, value)  # 调用前面写好的splitDataSet函数，对每个属性值划分一次数据集
            ...
            if IVa == 0:
                ...
                Gain_ratio = infoGain/IVa
            if Gain_ratio > bestGain_ratio:  # 如果此信息增益大于之前的记录的信息增益
                bestGain_ratio = Gain_ratio  # 更新最大的信息增益
    ...
    def majorityCnt(self, classList):
        ...
        sortedClassCount = sorted(classCount.items(), key=operator.itemgetter(1), reverse=True)  # 利用operator操作键值排序字典，reverse=True倒序
        return sortedClassCount[0][0]  # 取出第一个，即投票最多的


    # 创建树的函数代码
    def createTree(self, dataSet, labels):  # 数据集和标签列表
        ...
        for example in dataSet:
            if example[:-1] != x:
                ...
                break
        # 递归停止的第二个条件
        if len(dataSet[0]) == 1 or sign == 1:  # 遍历完所有特征时返回出现次数最多的类别（无法简单地返回唯一的类标签，使用前面的多数表决方法）
            return self.majorityCnt(classList)
        bestFeat = self.chooseBestFeatureToSplit(dataSet)  # 找出最佳数据集划分的特征属性的索引
        bestFeatLabel = labels[bestFeat]  # 获取最佳索引对应的值
        myTree = {bestFeatLabel: {}}
        labels = labels[:]
        del (labels[bestFeat])  # 删除最佳索引的列
        featValues = [example[bestFeat] for example in self.myDat]
        uniqueVals = set(featValues)  # 使用set集合去除重复的值，得到列表包含的所有属性值
        for value in uniqueVals:  # 遍历所有最佳划分的分类标签
            subLabels = labels[:]
            if not self.splitDataSet(dataSet, bestFeat, value):
                myTree[bestFeatLabel][value] =  self.majorityCnt(classList)
            else:
                myTree[bestFeatLabel][value] = self.createTree(self.splitDataSet(dataSet, bestFeat, value), subLabels)  # 递归调用
        return myTree
    
    def fit(self, x, y):
        data = np.hstack((x, y.reshape(-1, 1))).tolist()
        self.myTree = self.createTree(data, self.labels)
        return self.myTree
    ...
    def predict(self, x):
        y_pre = []
        for i in x:
            y_pre.append(self.classify(self.myTree, self.labels, i))
